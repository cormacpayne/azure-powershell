...
namespace Microsoft.Azure.Commands.Blueprint.Test.CmdletTests
{
    [TestClass]
    public class GetAzureBlueprintTest : RMTestBase
    {
        ...
        {
            // Setup
            var mgList = new[] { "AzBlueprint" };

            this.mockBlueprintClient.Setup(f => f.ListBlueprints(mgList)).Returns((IEnumerable<string> a) => new List<PSBlueprint>());

            // Test
            this.cmdlet.ManagementGroupId = mgList[0];
            this.cmdlet.ExecuteCmdlet();

            // Assert
            this.mockBlueprintClient.Verify(f => f.ListBlueprints(mgList), Times.Once());
        }
